...
import { User, Ruler, Weight, Sparkles, Loader2, ClipboardList, Utensils, RotateCcw, Trash2 } from 'lucide-react';
...
export default function More() {
    ...
    // Save plan whenever it's updated
    useEffect(() => {
        if (plan) {
    ...
    const clearPlan = () => {
        if (window.confirm(language === 'ar' ? 'هل تريد مسح الخطة الحالية؟' : 'Clear current plan?')) {
            setPlan(null);
        }
    };
    ...
    return (
        <div className="min-h-screen bg-black pb-24 text-white">
            ...
            <div className="p-6 max-w-md mx-auto space-y-8">
                ...
                >
                    <div className="absolute -inset-0.5 bg-gradient-to-r from-orange-500 to-red-600 rounded-[2rem] blur opacity-20 group-hover:opacity-30 transition duration-1000"></div>
                    <div className="relative bg-zinc-900 border border-zinc-800 rounded-[2rem] p-8 space-y-8">
                        <div className={`flex items-center gap-2 mb-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
                            <div className="w-2 h-8 bg-orange-500 rounded-full" />
                            <h2 className="text-xl font-bold tracking-tight">{language === 'ar' ? 'بيانات البطل' : 'Champion Stats'}</h2>
                        </div>
